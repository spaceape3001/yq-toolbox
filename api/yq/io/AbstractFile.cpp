...
namespace yq {
    ...
    std::error_code  AbstractFile::load(const filesystem_path_t& ipath)
    {
        if(!read_enabled())
    ...
    //bool            AbstractFile::load(u8_istream&fin, const filesystem_path_t& fp)
    std::error_code  AbstractFile::load(std::istream&fin, const filesystem_path_t& fp)
    {
        if(!read_enabled())
            return errors::not_read_enabled();
    ...
    std::error_code  AbstractFile::load(ByteArray&&data, const filesystem_path_t& fp)
    {
        if(!read_enabled())
            return errors::not_read_enabled();
    ...
    std::error_code  AbstractFile::save_as(const filesystem_path_t& oPath)
    {
    ...
    std::error_code  AbstractFile::save_to(const filesystem_path_t& oPath) const
    {
        if(!write_enabled())
        ...
        filesystem_path_t   tmp  = oPath.string() + ".tmp";
        int         n   = 0;
    ...
    void            AbstractFile::set_file(const filesystem_path_t&p)
    {
        if(can_change_to(p))
            m_file = p;
    }
