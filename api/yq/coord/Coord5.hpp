...
namespace yq {
    ...
    */
    ...
    struct Coord<T,5> {
        ...
        constexpr Coord(all_k, T _v) noexcept : Coord<T,5>(_v, _v, _v, _v, _v) {}
        
        //! Zero initializing constructor
        constexpr Coord(zero_k) noexcept : Coord<T,5>(ALL, zero_v<T>) {}
    ...
    template <typename T>
    constexpr Coord5<T>    coord(T i, std::type_identity_k<T> j, std::type_identity_k<T> k, std::type_identity_k<T> l, std::type_identity_k<T> m ) noexcept
    {
        return { i, j, k, l, m };
    }
