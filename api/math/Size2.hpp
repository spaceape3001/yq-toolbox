...
namespace yq {
    ...
    */
    ...
    template <typename T>
    AllComponents<Size2<T>>   all(const Size2<T>& val)
    {
        return AllComponents<Size2<T>>(val);
    }
    
    template <typename T>
    AllComponents<Size2<T>>   elem(const Size2<T>& val)
    {
        return AllComponents<Size2<T>>(val);
    }

    template <typename T>
    AnyComponents<Size2<T>>   any(const Size2<T>& val)
    {
        return AnyComponents<Size2<T>>(val);
    }
}
