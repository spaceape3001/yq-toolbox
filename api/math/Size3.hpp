...
namespace yq {
    ...
    */
    ...
    template <typename T>
    AllComponents<Size3<T>>   all(const Size3<T>& val)
    {
        return AllComponents<Size3<T>>(val);
    }
    
    template <typename T>
    AllComponents<Size3<T>>   elem(const Size3<T>& val)
    {
        return AllComponents<Size3<T>>(val);
    }

    template <typename T>
    AnyComponents<Size3<T>>   any(const Size3<T>& val)
    {
        return AnyComponents<Size3<T>>(val);
    }
}
