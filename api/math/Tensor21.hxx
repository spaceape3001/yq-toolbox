...
namespace yq {
    ...
    template <typename T>
        template <typename U>
    std::vector<Vector2<trait::product_t<T,U>>>    Tensor21<T>::operator*(std::span<const Vector1<U>> bs) const
    {
        return transform(bs, [&](const Vector1<U>& b) -> Vector2<trait::product_t<T,U>> {
            return *this * b;
        });
    }
